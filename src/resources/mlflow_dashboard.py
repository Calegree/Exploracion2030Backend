...
@swag_from('../flasgger/mlflow_prediction_counts.yml')
...
@swag_from('../flasgger/mlflow_runs.yml')
def mlflow_runs():
    try:
...
@swag_from('../flasgger/mlflow_confusion_matrix_current.yml')
...
@swag_from('../flasgger/mlflow_confusion_matrix.yml')
...
# Debug: verificar que los ficheros YML existen al importar el módulo
for _f in ['mlflow_prediction_counts.yml', 'mlflow_runs.yml', 'mlflow_confusion_matrix_current.yml', 'mlflow_confusion_matrix.yml']:
    p = os.path.join(YML_DIR, _f)
    if not os.path.exists(p):
        print(f"[mlflow_dashboard] WARNING: YAML file not found: {p}")
    else:
        print(f"[mlflow_dashboard] YAML found: {p}")
...
def prediction_counts():
        ...
        ---
        get:
            summary: Obtener contador de predicciones (Morchella / No Morchella)
            responses:
                "200":
                    description: Objeto con los contadores de predicciones
                    schema:
                        type: object
                        properties:
                            morchella:
...
def confusion_matrix_current():
    ...
    ---
    get:
      summary: Matriz de confusión del modelo activo
      responses:
        "200":
          description: Matriz de confusión o contadores de predicción
        "404":
          description: No se encontró información relevante
    """
    info = _load_model_info()
    run_id = info.get('active_run_id')
    if not run_id:
...
def mlflow_confusion_matrix_run_id(run_id):
    ...
    ---
    get:
      summary: Obtener matriz de confusión de un run de MLflow
      parameters:
        - name: run_id
          in: path
          required: true
          type: string
          description: ID del run de MLflow
      responses:
        "200":
          description: Matriz de confusión en formato JSON
        "404":
          description: No existe la matriz para el run solicitado
    """
    try:
        from mlflow.tracking import MlflowClient
    except Exception as e:
